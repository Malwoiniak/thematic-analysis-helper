...
df = [pd.read_csv(f) for f in glob.glob('libdem scrape/libdem final/*.csv')]

#Get data for words frequency distribution plot
df1 = [pd.read_csv(f) for f in glob.glob('libdem scrape/*.csv')]
...
fig3.update_layout(height=700, width=700,
                  title_text='<b>sentiment in libdemchild.com in all years of blog activity</b>', plot_bgcolor='blanchedalmond')
...
fig4.update_layout(title_text='<b>libdemchild.com sentiment in all years of blog activity</b>', plot_bgcolor='blanchedalmond', height=600, margin=go.layout.Margin(
                    l=50,
                    r=50,
                    b=100,
                    t=150,
...
app.title = 'Year in numbers at libdemchild.com'

app.layout = html.Div(children=[
    html.H2('Year in numbers at libdemchild.com', style={
        'textAlign': 'center'
    }),
...
def update_graph(year):
    ...
    fig0 = px.scatter(df[i], 
        ...
        title='<b>libdemchild.com: posts in {0}</b>'.format(year))
